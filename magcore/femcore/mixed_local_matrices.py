...
def local_vector_source_rhs(
    ...
    """
    Локальная правая часть от векторного токового источника:

        F_i = ∫_T J_s(x) · w_i(x) dV

    где:
    - J_s(x) — заданный векторный источник
    - w_i — локальные edge-базисные функции Nedelec
    """
...
def local_grad_p_coupling_matrix(
    ...
    """
    Локальная матрица связи mixed-постановки:

        G_ij = ∫_T (grad phi_j) · w_i dV

    где:
    - w_i     — локальные edge-базисы для A
    - phi_j   — скалярные P1-базисные функции
    - grad phi_j постоянен внутри аффинного тетраэдра
    """
    q = get_tetra_quadrature(quadrature_order)
    amap = AffineTetraMap(mesh.cell_vertices(cell_idx))
    detJ = amap.jacobian_determinant()

    grads_phi = amap.physical_barycentric_gradients()  # shape (4,3)
...
def local_scalar_mass_matrix(
    ...
    """
    Локальная скалярная mass-матрица для P1-базиса:

        M_ij = ∫_T beta * phi_i * phi_j dV

    На этапе A1.2 это служебный/диагностический блок.
    """
    if beta <= 0.0:
...
def local_curlcurl_block(
    ...
    """
    Тонкая обёртка для curl-curl блока A-A в mixed-постановке.
    """
