...
def local_vector_source_rhs(mesh: TetraMesh, cell_idx: int, J_fn, quadrature_order: int = 2) -> np.ndarray:
...
def local_grad_p_coupling_matrix(mesh: TetraMesh, cell_idx: int, quadrature_order: int = 2) -> np.ndarray:
    q = get_tetra_quadrature(quadrature_order)
    amap = AffineTetraMap(mesh.cell_vertices(cell_idx))
    detJ = amap.jacobian_determinant()
...
def local_scalar_mass_matrix(mesh: TetraMesh, cell_idx: int, beta: float = 1.0, quadrature_order: int = 2) -> np.ndarray:
    if beta <= 0.0:
...
def local_curlcurl_block(mesh: TetraMesh, cell_idx: int, nu: float, quadrature_order: int = 1) -> np.ndarray:
    return local_curlcurl_matrix(mesh=mesh, cell_idx=cell_idx, nu=nu, quadrature_order=quadrature_order)
