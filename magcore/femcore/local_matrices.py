...
def local_curlcurl_matrix(mesh: TetraMesh, cell_idx: int, nu: float, quadrature_order: int = 1) -> np.ndarray:
    if nu <= 0.0:
...
def local_mass_matrix(mesh: TetraMesh, cell_idx: int, alpha: float, quadrature_order: int = 2) -> np.ndarray:
    if alpha <= 0.0:
...
def local_rhs_vector(mesh: TetraMesh, cell_idx: int, f_fn, quadrature_order: int = 2) -> np.ndarray:
    q = get_tetra_quadrature(quadrature_order)
    amap = AffineTetraMap(mesh.cell_vertices(cell_idx))
    detJ = amap.jacobian_determinant()
